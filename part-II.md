...
[BlueStore — движок хранения]
        ↓
```dot
digraph BlueStoreLayers {
    rankdir=TB;
    bgcolor="white";
    fontname="Arial";
    fontcolor="black";
    node [fontname="Arial", fontcolor="black", style="filled,rounded", shape="box", penwidth=1.5];
    edge [fontname="Arial", fontcolor="black", color="#555555", penwidth=1.5, arrowsize=0.8];

    rocksdb [label="RocksDB\n(метаданные)", fillcolor="#FFCDD2", tooltip="key-value БД для метаданных объектов (OID → расположение на диске)"];
    bluefs  [label="BlueFS\n(мелкие файлы БД)", fillcolor="#BBDEFB", tooltip="мини-файловая система для RocksDB"];
    blockdev [label="BlockDevice\n(сырой раздел)", fillcolor="#C8E6C9", tooltip="raw-доступ к диску (никакой XFS/ext4!)"];

    rocksdb -> bluefs  [label="  key-value БД для\n  метаданных объектов\n  (OID → расположение на диске)", fontsize=9];
    bluefs  -> blockdev [label="  мини-файловая система\n  для RocksDB", fontsize=9];
}
```
...
digraph WALDBPlacement {
    ...
    subgraph cluster_hdd {
        ...
        hdd_data [label="Данные объектов\n(18 ТБ)", fillcolor="#FFECB3"];
    }

    subgraph cluster_nvme {
        label="Быстрый NVMe 512 ГБ";
        fontname="Arial";
        fontcolor="black";
        fontsize=12;
        bgcolor="#E8EAF6";
        ...
        nvme_wal [label="WAL (2 ГБ)\nжурнал", fillcolor="#C5CAE9"];
        nvme_db  [label="DB (30 ГБ)\nметаданные", fillcolor="#9FA8DA"];
    }

    nvme_wal -> nvme_db [style=invis];
}
```
