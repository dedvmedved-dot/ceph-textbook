...
digraph {
    ...
    client [label="Клиент\n(k8s pod)" fillcolor="#90CAF9" fontsize=11]
    agent [label="Vault Agent\n(injector / sidecar)" fillcolor="#A5D6A7" fontsize=11]
    ceph [label="Ceph\nCluster" fillcolor="#CE93D8" fontsize=11]
    vault [label="Vault\nServer" fillcolor="#FFCC80" fontsize=11]

    client -> agent [label="  запрос" color="#1B5E20" fontcolor="#1B5E20" penwidth=1.5]
    agent -> ceph [label="  динамический\n  ключ" color="#4A148C" fontcolor="#4A148C" penwidth=1.5]
    agent -> vault [label="  аутентификация\n  + генерация ключа" color="#E65100" fontcolor="#E65100" penwidth=1.5]
}
```
