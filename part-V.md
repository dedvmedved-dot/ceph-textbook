...
```
                ...
                └───────────┬─────────────┘
                    ...
                    └───┬─────────┬──┘
                        │         │
                 < min_alloc      >= min_alloc
                 (обычно 64KB)    (обычно 64KB)
                     │                 │
              ┌──────▼──────┐   ┌─────▼──────────┐
              │ Deferred     │   │ Есть свободный  │
              │ Write        │   │ непрерывный     │
              │ (отложенная) │   │ блок?           │
              └──────────────┘   └──┬──────────┬──┘
                                    │          │
...
```
Клиент: read(object_X, offset=4096, length=4096)
    │
    ▼
┌──────────────────────────────────────────────┐
│ 1. OSD получает запрос                      │
└──────────────────┬───────────────────────────┘
                   ▼
┌──────────────────────────────────────────────┐
│ 2. Поиск в BlueStore Cache (RAM)             │
│    ┌──────────────────────────┐              │
│    │ Onode Cache (объекты)    │ ← если есть: │
│    │ Buffer Cache (данные)    │    возврат    │
│    └──────────────────────────┘    без диска  │
└──────────────────┬───────────────────────────┘
                   ▼ (промах кеша)
┌──────────────────────────────────────────────┐
│ 3. RocksDB lookup: по OID найти              │
│    ○ Extent Map (какие блоки на диске)       │
│    ○ Контрольные суммы (SHA256)              │
│    ○ Состояние (WAL или Direct)              │
└──────────────────┬───────────────────────────┘
                   ▼
┌──────────────────────────────────────────────┐
│ 4. Чтение с диска:                           │
│    ○ Если данные в WAL → читаем из WAL       │
│    ○ Если данные в основном разделе → оттуда  │
│    ○ После чтения — проверка checksum        │
└──────────────────┬───────────────────────────┘
                   ▼
┌──────────────────────────────────────────────┐
│ 5. Обновление кеша (read-ahead) + возврат    │
└──────────────────────────────────────────────┘
```
...
Запись метаданных:
    ...
    ▼
┌──────────┐    ┌──────────────┐    ┌────────────────┐
│  MemTable│───►│  WAL (свой)  │───►│ Уровни на диске │
│ (в RAM)  │    │  (на NVMe)   │    │ L0, L1, L2... │
└──────────┘    └──────────────┘    └────────────────┘
    │                                      │
    │ При заполнении MemTable (64MB)       │ L0 → L1 объединяются
    │ сбрасывается в SST-файлы            │ (Compaction)
    ▼                                      ▼
  L0: несортированные SST             L1–L6: сортированные SST
  (быстрая запись)                    (быстрое чтение)
```
...
│  │  CPU 0-15    │    (медленный)      │  CPU 16-31   │
│  │  RAM: 128 GB │                     │  RAM: 128 GB │
│  │  NVMe 0,1    │                     │  NVMe 2,3    │
│  │  Eth: mlx5_0 │                     │  Eth: mlx5_1 │
│  └──────────────┘                     └──────────────┘
│                                                      │
│  Если OSD на NUMA 0 обращается к памяти NUMA 1:      │
│  Latency: +50–100%, Throughput: −30–50%              │
└─────────────────────────────────────────────────────┘
```
